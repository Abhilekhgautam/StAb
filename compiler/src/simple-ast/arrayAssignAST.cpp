  ...
  llvm::Value* ArrayAssignAST::codegen(Scope* s){
	...
	if (val->getType() == "int"){
	 ...
	 // get llvm::AllocaInst* out of optional
         auto arrayVar = std::get_if<llvm::AllocaInst*>(&(arrayVarOptional.value()));

	 std::vector<llvm::Value *> indices;
	 llvm::Value* zeroIndex = llvm::ConstantInt::get(*TheContext, llvm::APInt(32, 0));
         indices.emplace_back(zeroIndex); // Array pointer
         indices.emplace_back(index->codegen(s)); // Index

	 llvm::Value *elementPtr = Builder->CreateGEP((*arrayVar)->getAllocatedType(), *arrayVar, indices, "elementPtr");
